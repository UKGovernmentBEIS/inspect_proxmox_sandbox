...
class ProxmoxSandboxEnvironment(SandboxEnvironment):
    ...
    async def sample_cleanup(
        ...
        try:
            if any_vm_sandbox_environment is not None:
                # Use per-host concurrency lock to match sample_init
                host = instance.host if instance else "unknown"
                async with concurrency(f"proxmox-{host}", 1):
    ...
    async def task_cleanup(
        ...
        # Disabled for q4_scaling branch - we do all cleanup in sample_cleanup
        return
