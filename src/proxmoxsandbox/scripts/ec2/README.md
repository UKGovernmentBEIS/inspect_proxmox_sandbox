...
dnsmasq hands itself out as the DHCP-supplied resolver and `SubnetConfig` cannot point
guests elsewhere, so a drop would hang every lookup in the guest until its own resolver
timed out.
...
4. **A security group with no inbound rules** (SSM needs none) and outbound 443 to the
   endpoints' security group — and nothing else attached to the instance. An instance's
   effective rules are the union of every group attached to it, so one extra permissive
   group (the VPC's `default`, say, which allows all outbound) undoes this item.
