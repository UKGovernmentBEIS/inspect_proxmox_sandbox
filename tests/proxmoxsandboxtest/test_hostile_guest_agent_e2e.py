"""Check provider behavior against a canned guest-agent replacement in a fresh VM."""
...
# Proxmox synchronizes and pings before each command; those replies stay valid
# so each test reaches its intended command response.
SHIM_SOURCE = r"""
...
# The helper executes nothing; all command output and exit-code files contain 0.
# Overrides change one part of this otherwise valid conversation.
...
async def hostile_env(request):
    ...
    template = os.getenv("PROXMOX_LINUX_TEMPLATE_TAG")
    config = (
        ProxmoxSandboxEnvironmentConfig(
            vms_config=(
                VmConfig(
                    vm_source_config=VmSourceConfig(existing_vm_template_tag=template),
                    nics=(),
                ),
            ),
        )
        if template
        else ProxmoxSandboxEnvironmentConfig()
    )
...
TAMPER_SCENARIOS = {
    ...
    "valid_prefix_invalid_tail": {
        "guest-file-read": {
            "return": {"count": 48, "buf-b64": "A" * 64 + "!", "eof": True}
        }
    },
    "content_not_string": {
...
async def test_exec_without_timeout_is_bounded(hostile_env, monkeypatch):
    ...
    with pytest.raises(TimeoutError):
        # Keep a stalled provider from hanging the live test.
...
async def test_exec_with_timeout_is_bounded_when_agent_stalls(hostile_env):
    ...
    started = time.monotonic()
    with pytest.raises(TimeoutError):
        await asyncio.wait_for(hostile_env.exec(["true"], timeout=5), 300)
